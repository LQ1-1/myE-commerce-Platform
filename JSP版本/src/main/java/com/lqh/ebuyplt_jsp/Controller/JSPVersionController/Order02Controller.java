...
import com.lqh.ebuyplt_jsp.Controller.OrderCheckPack.OrderStatus;
import com.lqh.ebuyplt_jsp.Controller.OrderPack2.*;
...
public class Order02Controller
{
    ...
    @POST
    @Path("/OrderConfirm_NewDeliveryRecord")
    @Produces("text/plain")                                    //在Profile界面管理收货信息的时候，也可以使用这个api来添加新的收货记录
    public String OrderConfirm_NewDeliveryRecord(DeliveryInfo  deliveryInfo)           //用户发来收货地址信息
    {
        {
    ...
    @POST
    @Path("/OrderConfirm_OrderGenerate")
    @Produces("application/json")
    public ApiResult<OrderItem_Feedback> OrderConfirm_OrderGenerate(OrderItem  orderItem)
    {
        {
    ...
    @POST
    @Path("/OrderStatus_Update")
    @Produces("text/plain")              //用户通过这个更新是否支付的状态，配送的时候也可以通过这个来更新状态
    public String OrderStatus_Update(OrderStatusUpdate_jsonGet orderStatusUpdate)
    {
        {
    ...
    @POST
    @Path("/OrderSingularProductDeliveryStatus_Update")
    @Produces("text/plain")
    public String OrderSingularProductDeliveryStatus_Update(OrderSingularProductDeliveryStatusUpdate_jsonGet newPStatus)
    {
        {
    ...
    @Path("/OrderCancelled")
    @Produces("application/json")
    public ApiResult<OrderCancelled_jsonSend>OrderCancelled(OrderCancelled_jsonGet orderCancelled)         //处理订单取消的接口
    {
