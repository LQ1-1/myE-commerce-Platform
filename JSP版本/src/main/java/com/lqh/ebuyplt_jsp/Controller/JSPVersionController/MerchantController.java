...
import com.lqh.ebuyplt_jsp.Controller.MerchantPack.*;
...
public class MerchantController
{
    ...
    public ApiResult<ProductOnSale_jsonSend> ProductOnSale(ProductOnSale_jsonGet productOnSale)   //这里应该返回订单号回去的
    {
...
//            Connection con= DriverManager.getConnection(url,user,password);
    ...
    @POST
    @Path("/ProductImagesUpload")
    @Produces("text/plain")
    public String ProductImagesUpload(ProductImagesUpload form)
    {
    ...
    private boolean ProductImagesUploadResult(ProductImagesUpload form)
    {
        ...
        // 获取封面
        Part cover = form.getcover();

        // 获取展示图
        Part[] gallery = form.getgallery();
        ...
        try
        {
            //处理缩略图
            String coverUrl="";
            if(cover!=null && !(cover.getSize()==0L))
            {
            ...
            {
                for(Part galleryFile : gallery)
                {
                    if(!(galleryFile.getSize()==0L))
                    {
